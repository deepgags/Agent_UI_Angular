            ...
            if(result && result.data && result.data.length > 0)
            {
              return result.data.map((property:any)=>
              {
                return {
                  ...
                  Media: property.Media.filter((x:any)=>x.Media_type && x.Media_status=="Active" && x.Media_category.includes('Photo') && x.ImageSize_description.includes('Large')),
                ...
                const propertyModel: PropertyModel =
                   {
                    ...
                    UnitNumber: property.UnitNumber,
                    UnparsedAddress: property.UnparsedAddress,
                    Media: property.Media.filter((x:any)=>x.Media_type && x.Media_category.includes('Photo')),
                    BuildingAreaTotal: property.BuildingAreaTotal,
                    BuildingAreaUnits: property.BuildingAreaUnits,
                    TotalRecords: result.total,
                    ListOfficeName: property.ListOfficeName,
