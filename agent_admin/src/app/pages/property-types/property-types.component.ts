...
export class PropertyTypesComponent implements OnInit {
  ...
  loadItems() {
    ...
    this.apiClient.getPaginated<PropertyType>('/property-types', { page: 1, pageSize: 10 }).subscribe({ next: (res) => { this.items.set(res.data); this.loading.set(false); }, error: () => this.loading.set(false) });
  }
